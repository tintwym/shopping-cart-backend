        ...
        synchronized (sessionId.intern()) {
            transactionTemplate.executeWithoutResult(status -> {
                ...
                for (Map.Entry<UUID, Integer> e : qtyByProduct.entrySet()) {
                    Product fresh = productRepository.findById(e.getKey())
                ...
                for (Map.Entry<UUID, Integer> e : qtyByProduct.entrySet()) {
                    Product fresh = productRepository.findById(e.getKey()).orElseThrow();
                    int qty = e.getValue();
    ...
    private Product resolveProduct(LineItem lineItem) {
        ...
        if (stripeProduct != null && stripeProduct.getMetadata() != null) {
            ...
            if (internalId != null && !internalId.isBlank()) {
                return productRepository.findById(UUID.fromString(internalId))
